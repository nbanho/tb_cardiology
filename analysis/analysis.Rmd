...
df <- readRDS("data-clean/echo.rds") %>%
  ...
  mutate(
    ...
    bmi_cat =
      ifelse(is.na(bmi), "Unknown",
        ifelse(bmi > 30, "Obese (BMI > 30kg/m2)",
          ifelse(bmi < 18, "Underweight (BMI < 18kg/m2)",
            "Normal (18kg/m2 <= BMI <= 30kg/m2)"
          )
        )
      ),
...
```{r follow-up-time}
fup_times <- df %>%
  dplyr::select(record_id, event_time, date_of_tte) %>%
  pivot_wider(names_from = event_time, values_from = date_of_tte) %>%
  mutate(
    across(c(Start, End, Post), as.Date),
    fup_end = as.numeric(End - Start),
    fup_post = as.numeric(Post - End)
  )

fup_times_by_event <- fup_times %>%
  summarise(across(c(fup_end, fup_post), list(
    median = function(x) median(x, na.rm = TRUE),
    lower = function(x) quantile(x, .25, na.rm = TRUE),
    upper = function(x) quantile(x, .75, na.rm = TRUE)
  ))) %>%
  mutate_all(round) %>%
  mutate_all(as.integer)

sprintf(
  "Median follow-up time overall: %i (IQR %i to %i)",
  as.integer(median(c(fup_times$fup_end, fup_times$fup_post), na.rm = TRUE)),
  as.integer(quantile(c(fup_times$fup_end, fup_times$fup_post), .25, na.rm = TRUE)),
  as.integer(quantile(c(fup_times$fup_end, fup_times$fup_post), .75, na.rm = TRUE))
)

sprintf(
  "Median follow-up time between end and start of treatment: %i (IQR %i to %i)",
  fup_times_by_event$fup_end_median,
  fup_times_by_event$fup_end_lower,
  fup_times_by_event$fup_end_upper
)
sprintf(
  "Median follow-up time between end and start of treatment: %i (IQR %i to %i)",
  fup_times_by_event$fup_post_median,
  fup_times_by_event$fup_post_lower,
  fup_times_by_event$fup_post_upper
)
```
...
save_plot(
  flow_post_pl,
  pdf_file = "results/sankey-post.tif",
  w = 16, h = 10
)
```
...
df_sub <- df %>%
  dplyr::filter(event_time %in% c("Start", "End")) %>%
  mutate(
    event_time = factor(
      event_time,
      levels = c("Start", "End")
  ...
  mutate(
    across(c(-record_id), ~ as.integer(.x) - 1)
  )
...
df_imp_long <- complete(df_imp, action = "long") %>%
  pivot_longer(
    cols = ends_with("_End") | ends_with("_Start"),
    names_to = c(".value", "event_time"),
    names_pattern = "(.*)_(.*)"
  ) %>%
  mutate(event_time = ifelse(event_time == "Start", 0, 1))
...
cc_prop <- df_sub %>%
  ...
  ungroup() %>%
  mutate(event_time = factor(event_time, levels = c("Start", "End"))) %>%
...
save_plot(
  pl_comp,
  pdf_file = "results/pericardial-changes.tif",
  w = 16, h = 8
)
```

```{r, fibrinstrand_echodens}
prop.test(
  x = c(8, 0),
  n = c(286, 88),
  correct = FALSE
...
for (out in sub_outcomes) {
  ...
  mod <- glm(form, data = df_sub_long, family = binomial())
  ...
  for (pred in predictors) {
    # counts
    counts_start <- table(df_sub_long[df_sub_long[["event_time"]] == "Start" & df_sub_long[[out]] == 1, pred])
    counts_end <- table(df_sub_long[df_sub_long[["event_time"]] == "End" & df_sub_long[[out]] == 1, pred])
    ...
    uni_mod <- glm(uni_form, data = df_sub_long, family = binomial())
    ...
    cc_assoc <- rbind(
      ...
      tibble(
        ...
        outcome = out,
        predictor = pred,
        category = names(counts_start),
        count = counts_start,
        percentage = counts_start / sum(counts_start),
        mean = exp(coef_start),
        lower = exp(coef_start - qnorm(.975) * se_start),
        upper = exp(coef_start + qnorm(.975) * se_start),
        pvalue = pval_start
      ...
      tibble(
        ...
        outcome = out,
        predictor = pred,
        category = names(counts_end),
        count = counts_end,
        percentage = counts_end / sum(counts_end),
        mean = exp(coef_end),
        lower = exp(coef_end - qnorm(.975) * se_end),
        upper = exp(coef_end + qnorm(.975) * se_end),
        pvalue = pval_end
...
cc_assoc <- cc_assoc %>%
  mutate(
    ...
    category = ifelse(type == "Univariable", category,
      ifelse(type == "Multivariable", predictor, category)
    ),
    category = recode(category, !!!predictor_labs),
    percentage = round(percentage * 100),
    count_perc = paste0(count, " (", percentage, ")")
  )
...
for (t in c("Start", "End")) {
  ...
  for (k in 1:length(sub_outcomes)) {
    ki <- ki + 1
    count_out <- sum(df_sub_long[sub_outcomes[k]] == 1 & df_sub_long$event_time == t, na.rm = TRUE)
    perc_out <- count_out / sum(!is.na(df_sub_long[df_sub_long$event_time == t, sub_outcomes[k]]))
    count_perc_out <- paste0(count_out, " (", round(100 * perc_out), ")")
    assoc_res[ki, ] <- c(
    ...
    for (i in 1:length(predictors)) {
      ki <- ki + 1
      assoc_res[ki, ] <- c(
        paste0("   ", predictor_levels[i, 1], " (reference)"),
        "",
        cc_assoc %>%
          filter(type == "Univariable") %>%
          filter(category == predictor_levels[i, 1]) %>%
          filter(time == t) %>%
          filter(!is.na(count_perc)) %>%
          filter(outcome == sub_outcomes[k]) %>%
          pull(count_perc),
        "",
        "",
        "", "",
      ...
      assoc_res[ki, ] <- unname(c(
        ...
        cc_assoc %>%
          filter(type == "Univariable") %>%
          filter(category == predictor_levels[i, 2]) %>%
          filter(time == t) %>%
          filter(!is.na(count_perc)) %>%
          filter(outcome == sub_outcomes[k]) %>%
          pull(count_perc),
