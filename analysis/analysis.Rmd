...
for (out in sub_outcomes) {
  ...
  for (pred in predictors) {
    # counts
    counts_start <- table(df_sub[df_sub[["event_time"]] == "Start" & df_sub[[out]] == 1, pred])
    counts_end <- table(df_sub[df_sub[["event_time"]] == "End" & df_sub[[out]] == 1, pred])
...
for (t in c("Start", "End")) {
  ...
  for (k in 1:length(sub_outcomes)) {
    ki <- ki + 1
    count_out <- sum(df_sub[sub_outcomes[k]] == 1 & df_sub$event_time == t, na.rm = TRUE)
    perc_out <- count_out / sum(!is.na(df_sub[df_sub$event_time == t, sub_outcomes[k]]))
